...
const GNOSIS_SAFE_NONCE = process.env.NONCE || 1;
...
async function main() {
  ...
  const executeCalldata = TimelockAdmin.interface.encodeFunctionData("execute",[
          proxyAddress, 0, upgradeCalldata, BYTES32_ZERO, BYTES32_ZERO
  ]);


  let SafeOperation = 0; // 0 is Call, 1 is DelegateCall
  const [hash] = await multisigInstance.functions.getTransactionHash(
      timelockAddress, 0, executeCalldata, SafeOperation, GAS_LIMIT, 1, 1, ADDRESS_ZERO, ADDRESS_ZERO, GNOSIS_SAFE_NONCE );
  ...
  const receipt = await multisigInstance.functions.execTransaction(
      timelockAddress, 0, executeCalldata, SafeOperation, GAS_LIMIT, 1, 1, ADDRESS_ZERO, ADDRESS_ZERO, signatures,
  );
